...
// 1️⃣ Definizione degli eventi
  ...
  const events = {
    "2025-10-12": { text: "Flashmob FCBD", url: "fcbd.html" , category: "Eventi"},
    "2025-10-16": {text: "Livello intermedio Kairos", category: "LezioneKairos" },
    ...
    while (dayCount <= daysInMonth) {
      ...
      for (let i = 0; i < 7; i++) {
        ...
        if ((dayCount === 1 && i < startDay) || dayCount > daysInMonth) {
          ...
          if (events[dayKey]) {
            cell.style.cursor = "pointer"; //mette il puntatore se c'è un evento
            cell.style.fontWeight = "bold"; //mette il grassetto se c'è un evento
            let eventObj = typeof events[dayKey] === "object" 
               ? events[dayKey] 
               : { text: events[dayKey], category: "altro" };

            cell.style.backgroundColor = categoryColors[eventObj.category];
